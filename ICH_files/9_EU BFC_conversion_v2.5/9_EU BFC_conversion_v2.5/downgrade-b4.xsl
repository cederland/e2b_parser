...
<!--
		...
		Status:			Current
		Author:			Nick Halsey (EU)
		Amendment:	Correction to handle UCUM dosage units that are not mapped to R2 codes, fix to drug start and end dates missing in some instances
...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:hl7="urn:hl7-org:v3" xmlns:mif="urn:hl7-org:v3/mif" exclude-result-prefixes="hl7 xsi xsl fo mif">
	...
	<xsl:template match="hl7:substanceAdministration" mode="drug-start-date">
		<xsl:choose>
			<xsl:when test="hl7:effectiveTime/hl7:comp[@xsi:type='IVL_TS']/hl7:low/@value">
			...
			<xsl:otherwise>
				<xsl:if test="hl7:effectiveTime[@xsi:type='IVL_TS']/hl7:low/@value">
					<xsl:call-template name="convertDate">
						<xsl:with-param name="elementName">drugstartdate</xsl:with-param>
						<xsl:with-param name="date-value" select="hl7:effectiveTime[@xsi:type='IVL_TS']/hl7:low/@value"/>
						<xsl:with-param name="min-format">CCYY</xsl:with-param>
						<xsl:with-param name="max-format">CCYYMMDD</xsl:with-param>
					</xsl:call-template>
				</xsl:if>
	...
	<xsl:template match="hl7:substanceAdministration" mode="drug-end-date">
		<xsl:choose>
			<xsl:when test="hl7:effectiveTime/hl7:comp[@xsi:type='IVL_TS']/hl7:high/@value">
				<xsl:call-template name="convertDate">
			...
			<xsl:otherwise>
				<xsl:if test="hl7:effectiveTime[@xsi:type='IVL_TS']/hl7:high/@value">
					<xsl:call-template name="convertDate">
						<xsl:with-param name="elementName">drugenddate</xsl:with-param>
						<xsl:with-param name="date-value" select="hl7:effectiveTime[@xsi:type='IVL_TS']/hl7:high/@value"/>
						<xsl:with-param name="min-format">CCYY</xsl:with-param>
						<xsl:with-param name="max-format">CCYYMMDD</xsl:with-param>
					</xsl:call-template>
				</xsl:if>
